...
import {deleteProfileImage, saveProfileImage} from "../../utils/imageProcessing";
    ...
    try{
        ...
        if(req.file){
            if(dbUser.profileImage){
                await deleteProfileImage(dbUser.profileImage.split("/").at(-1)!)
            }
            filename = await saveProfileImage({filename: req.file.filename, filePath: req.file.path})
            dbUser.profileImage = filename && (`${baseConfig.HOST}/static/${filename}`)
        }
...
export const removeProfileImage: RequestHandler  = async (req,res,next) => {
    try {
        // @ts-ignore
        const dbUser = await appDb.User.findByPk(req.user!.userId)
        if (!dbUser) {
            return next(new AssumptionViolationError({
                assumption: "User was able to login and get authenticated, but it's data is not in the app database.",
                whereInitiated: req.originalUrl,
