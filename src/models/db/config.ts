...
if (process.env.NODE_ENV === "development") {
    ...
    assert(
        ...
        "PROD database name is not set in env",
    );
} else if (process.env.NODE_ENV === "test") {
    assert(
        process.env.DB_DATABASE_TEST,
        "TEST database name is not set in env",
    );
}
...
const caCertFormatted = process.env.CA_CERT.replace(/<NEWLINE>/g, "\n");
// @ts-ignore
const config: Config = {
    development: {
